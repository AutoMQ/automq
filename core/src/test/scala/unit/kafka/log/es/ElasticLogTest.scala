...
class ElasticLogTest {
    ...
    val elasticLogManager: ElasticLogManager = new ElasticLogManager(new MemoryClient())
    val producerStateManagerConfig = new ProducerStateManagerConfig(kafka.server.Defaults.ProducerIdExpirationMs)
    val log: ElasticLog = createElasticLogWithActiveSegment(config = LogTestUtils.createLogConfig())
    ...
    /**
     * This test is intended to check the behavior of the log when the log directory is renamed.
     * However, ElasticLog does not rely on any directory structure. Path check is thus skipped.
     */
    @Test
