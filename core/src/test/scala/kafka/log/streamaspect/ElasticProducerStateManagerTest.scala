...
import org.apache.kafka.common.errors.{InvalidProducerEpochException, InvalidTxnStateException, OutOfOrderSequenceException, TransactionCoordinatorFencedException}
...
class ElasticProducerStateManagerTest {
    ...
    private var snapshotsMap: util.NavigableMap[java.lang.Long, ByteBuffer] = _

    @BeforeEach
    def setUp(): Unit = {
        logDir = TestUtils.tempDir()
        snapshotsMap = new ConcurrentSkipListMap[java.lang.Long, ByteBuffer]()
        stateManager = new ElasticProducerStateManager(partition, logDir, maxTransactionTimeoutMs,
            producerStateManagerConfig, time, snapshotsMap, _ => CompletableFuture.completedFuture(null))
    }
    ...
    def testHasLateTransactionInitializedAfterReload(): Unit = {
        ...
        val reloadedStateManager = new ElasticProducerStateManager(partition, logDir, maxTransactionTimeoutMs,
            producerStateManagerConfig, time, snapshotsMap, _ => CompletableFuture.completedFuture(null))
    ...
    def testRecoverFromSnapshotUnfinishedTransaction(): Unit = {
        ...
        stateManager.takeSnapshot()
        val recoveredMapping = new ElasticProducerStateManager(partition, logDir,
            maxTransactionTimeoutMs, producerStateManagerConfig, time, snapshotsMap, _ => CompletableFuture.completedFuture(null))
    ...
    def testDeleteSnapshotsBefore(): Unit = {
        ...
        append(stateManager, producerId, epoch, 1, 1L)
        stateManager.takeSnapshot()
        assertEquals(1, snapshotsMap.size())
        assertEquals(Set(2), currentSnapshotOffsets)

        append(stateManager, producerId, epoch, 2, 2L)
        stateManager.takeSnapshot()
        assertEquals(2, snapshotsMap.size())
        ...
        assertEquals(0, snapshotsMap.size())
    ...
    def testSequenceNotValidatedForGroupMetadataTopic(): Unit = {
        ...
        val stateManager = new ElasticProducerStateManager(partition, logDir,
            maxTransactionTimeoutMs, producerStateManagerConfig, time, snapshotsMap, _ => CompletableFuture.completedFuture(null))
    ...
    private def currentSnapshotOffsets: Set[Long] = {
        snapshotsMap.keySet().asScala.toSet.map(Long2long)
    }
