...
public class DynamicTelemetryAdapter implements Reconfigurable {
    ...
    public void validateReconfiguration(Map<String, ?> configs) throws ConfigException {
        if (configs.containsKey(AutoMQTelemetryManager.EXPORTER_URI_CONFIG)) {
            Object value = configs.get(AutoMQTelemetryManager.EXPORTER_URI_CONFIG);
            String uri;
            ...
            if (uri != null && !uri.isBlank()) {
                telemetryManager.validateExporterUri(uri);
            }
        }
    ...
    public void reconfigure(Map<String, ?> configs) {
        boolean changed = telemetryManager.mayApplyConfigChanges(configs);
        if (changed) {
