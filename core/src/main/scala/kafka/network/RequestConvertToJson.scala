...
object RequestConvertToJson {
  def request(request: AbstractRequest): JsonNode = {
    request match {
      ...
      case req: CommitSSTObjectRequest => CommitSSTObjectRequestDataJsonConverter.write(req.data, request.version)
  ...
  def response(response: AbstractResponse, version: Short): JsonNode = {
    response match {
      ...
      case res: CommitSSTObjectResponse => CommitSSTObjectResponseDataJsonConverter.write(res.data, version)
