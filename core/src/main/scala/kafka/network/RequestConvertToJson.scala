...
object RequestConvertToJson {
  def request(request: AbstractRequest): JsonNode = {
    request match {
      ...
      case req: ListAllControllerBrokerIdsRequest => ListAllControllerBrokerIdsRequestDataJsonConverter.write(req.data, request.version)
  ...
  def response(response: AbstractResponse, version: Short): JsonNode = {
    response match {
      ...
      case res: ListAllControllerBrokerIdsResponse => ListAllControllerBrokerIdsResponseDataJsonConverter.write(res.data, version)
      // Kafka on S3 inject end
