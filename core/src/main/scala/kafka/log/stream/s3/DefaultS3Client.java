...
public class DefaultS3Client implements Client {
    ...
    public void start() {
        ...
        this.registerRootObjectStorageHandler();
    ...
    protected void registerRootObjectStorageHandler() {
        RootObjectStorageFactory root = new RootObjectStorageFactory(config.objectTagging(), networkInboundLimiter, networkOutboundLimiter, nodeCircuitStatusManagerStub(), objectManager);
        ObjectStorageFactory.instance().registerProtocolHandler("root", root::get);
    }

    protected CircuitBreaker.NodeCircuitStatusManagerStub nodeCircuitStatusManagerStub() {
        return status -> FutureUtil.failedFuture(new UnsupportedOperationException());
    }

    protected ObjectStorage newMainObjectStorage() {
        return ObjectStorageFactory.instance().builder()
            .buckets(config.dataBuckets())
            .extension("type", RootObjectStorageFactory.Type.MAIN)
