...
class GroupCoordinatorAdapter(
  ...
  override def getConsumerJoinAttempts(
    context: RequestContext,
    request: GetConsumerJoinAttemptsRequestData,
    bufferSupplier: BufferSupplier
  ): CompletableFuture[GetConsumerJoinAttemptsResponseData] = {
    FutureUtils.failedFuture(Errors.UNSUPPORTED_VERSION.exception(
      s"The old group coordinator does not support ${ApiKeys.GET_CONSUMER_JOIN_ATTEMPTS.name} API."
    ))
  }
