...
class ControllerApis(val requestChannel: RequestChannel,
  ...
  override def handle(request: RequestChannel.Request, requestLocal: RequestLocal): Unit = {
    try {
      val handlerFuture: CompletableFuture[Unit] = request.header.apiKey match {
        ...
        case ApiKeys.COMMIT_STREAM_SET_OBJECT => handleCommitSSTObject(request)
  ...
  def handleCommitSSTObject(request: RequestChannel.Request): CompletableFuture[Unit] = {
    val commitSSTObjectRequest = request.body[CommitStreamSetObjectRequest]
    val context = new ControllerRequestContext(request.context.header.data, request.context.principal,
      OptionalLong.empty())
    controller.commitStreamSetObject(context, commitSSTObjectRequest.data)
      .handle[Unit] { (result, exception) =>
        if (exception != null) {
          requestHelper.handleError(request, exception)
        } else {
