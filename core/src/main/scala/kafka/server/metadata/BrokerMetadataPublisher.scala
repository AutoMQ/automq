...
class BrokerMetadataPublisher(
  ...
  /**
   * Single thread partition op callback executor to keep orderly run callback.
   */
  val partitionOpCallbackExecutor = ThreadUtils.newSingleThreadExecutor("partition_op_callback", true)
    ...
    def callback(topicPartition: TopicPartition): Unit = {
      ...
      if (topicsDelta.topicWasDeleted(Topic.GROUP_METADATA_TOPIC_NAME)) {
        ...
        if (partitionRegistration != null && partitionRegistration.leader == brokerId) {
          groupCoordinator.onResignation(topicPartition.partition(), Option(partitionRegistration.leaderEpoch))
        }
      }
      ...
      try {
        ...
        if (topicsDelta.topicWasDeleted(topicPartition.topic())) {
          groupCoordinator.handleDeletedPartitions(
            mutable.Seq.apply(new TopicPartition(topicPartition.topic(), topicPartition.partition())),
