...
public class LocalFileObjectStorageTest {
    ...
    public void testList() throws ExecutionException, InterruptedException {
        ...
        assertEquals(
            ...
            objectStorage.list("").get().stream().map(ObjectStorage.ObjectPath::key).collect(Collectors.toList())
        );
        assertEquals(
            List.of("abc/def/100", "abc/def/101"),
            objectStorage.list("abc/def").get().stream().map(ObjectStorage.ObjectPath::key).collect(Collectors.toList())
        );
        assertEquals(
            List.of("abc/def/100", "abc/def/101"),
            objectStorage.list("abc/def/").get().stream().map(ObjectStorage.ObjectPath::key).collect(Collectors.toList())
        ...
        assertEquals(
            List.of("abc/def/100", "abc/def/101"),
            objectStorage.list("abc/def/1").get().stream().map(ObjectStorage.ObjectPath::key).collect(Collectors.toList())
        );
    }
