...
# Enhanced function to filter out heavy dependencies for Connect
should_include_file_for_connect() {
  local file="$1"
...
if [ -z "$UPGRADE_KAFKA_STREAMS_TEST_VERSION" ]; then
  ...
  do
    # Apply filtering to each jar in the directory instead of using wildcard
    for file in "$dir"/*;
    do
      if should_include_file_for_connect "$file"; then
...
do
  # Apply filtering to each jar in the directory
  for file in "$dir"/*;
  do
    if should_include_file_for_connect "$file"; then
      CLASSPATH="$CLASSPATH":"$file"
...
# AutoMQ modules
for automq_module in "automq-metrics" "automq-log-uploader"
do
  for file in "$base_dir"/${automq_module}/build/libs/*.jar;
  do
    if should_include_file_for_connect "$file"; then
      CLASSPATH="$CLASSPATH":"$file"
    fi
...
if [ "x$GC_LOG_ENABLED" = "xtrue" ]; then
  ...
  # it then becomes '9', '10', '11', '12', etc. for later releases
