...
#echo "KAFKA_JDK_COMPATIBILITY_OPTS = $KAFKA_JDK_COMPATIBILITY_OPTS"
...
if [ "x$DAEMON_MODE" = "xtrue" ]; then
  ...
  nohup "$JAVA" --add-opens=java.base/java.util=ALL-UNNAMED $KAFKA_HEAP_OPTS $KAFKA_JVM_PERFORMANCE_OPTS $KAFKA_GC_LOG_OPTS $KAFKA_JMX_OPTS $KAFKA_LOG4J_OPTS $KAFKA_JDK_COMPATIBILITY_OPTS -cp "$CLASSPATH" $KAFKA_OPTS "$@" > "$CONSOLE_OUTPUT_FILE" 2>&1 < /dev/null &
else
  exec "$JAVA" --add-opens=java.base/java.util=ALL-UNNAMED $KAFKA_HEAP_OPTS $KAFKA_JVM_PERFORMANCE_OPTS $KAFKA_GC_LOG_OPTS $KAFKA_JMX_OPTS $KAFKA_LOG4J_OPTS $KAFKA_JDK_COMPATIBILITY_OPTS -cp "$CLASSPATH" $KAFKA_OPTS "$@"
fi
