...
class KafkaService(KafkaPathResolverMixin, JmxMixin, Service):
    ...
    def start(self, add_principals="", nodes_to_skip=[], isolated_controllers_to_skip=[], timeout_sec=60, **kwargs):
        try:
            self.start0(add_principals=add_principals, nodes_to_skip=nodes_to_skip, isolated_controllers_to_skip=isolated_controllers_to_skip, timeout_sec=timeout_sec, **kwargs)
        except RemoteCommandError as e:
            self.logger.error("RemoteCommandError when starting Kafka service: %s", e)
            raise

    def start0(self, add_principals="", nodes_to_skip=[], isolated_controllers_to_skip=[], timeout_sec=60, **kwargs):
        """
