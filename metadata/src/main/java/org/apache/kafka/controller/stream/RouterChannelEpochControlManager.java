    ...
    private ControllerResult<Void> run0() {
        ...
        if (epoch == null) {
            ...
            if (time.milliseconds() - epoch.getLastBumpUpTimestamp() >= BUMP_EPOCH_INTERVAL) {
                ...
                newEpoch = new RouterChannelEpoch(newCommitedEpoch.orElse(epoch.getCommited()), epoch.getFenced() + 1, epoch.getCurrent() + 1, time.milliseconds());
            } else if (newCommitedEpoch.isPresent() && newCommitedEpoch.getAsLong() > epoch.getCommited()) {
